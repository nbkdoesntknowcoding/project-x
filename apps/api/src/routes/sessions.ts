...
export const sessionsRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.get('/api/sessions/:id/export', async (req, reply) => {
    ...
    const lines: string[] = [
      ...
      `**Cost:** $${session.totalCostUsd.toFixed(6)} | **Tool calls:** ${session.totalToolCalls}${durationMs !== null ? ` | **Duration:** ${Math.round(durationMs / 1000)}s` : ''}`,
      '',
      '## Timeline',
      '',
    ];
    ...
    lines.push('', `---`, `*Exported from Mnema AgentLens*`);
