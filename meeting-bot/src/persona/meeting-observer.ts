// meeting-bot/src/persona/meeting-observer.ts  (STEP 9 — Meeting-Mode System Prompt)
//
// A NEW persona — not the personal assistant persona from VAP. It is loaded into
// Pipecat as the `system_prompt` when the bot joins a meeting. In the VAP this is
// injected via media-worker/worker.py (ACTIVE_PERSONA_ID → persona/loader.py →
// OpenAILLMContext system message). For the meeting bot, build this string and
// hand it to the meeting pipeline as the system prompt.
export function buildMeetingPersona(context: {
  projectName?: string;
...
${context.meetingTitle ? `Meeting: ${context.meetingTitle}` : 'Meeting title not yet determined.'}
${context.projectName ? `Project: ${context.projectName}` : 'Project not yet identified — ask at meeting start.'}
${context.projectContext ? `Context: ${context.projectContext}` : ''}
</project_context>
